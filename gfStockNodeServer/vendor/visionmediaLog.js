...
var setting = require('./../cfg/setting')

var EventEmitter = require('events').EventEmitter;

var defaultDateFormat=setting.log.dateFormat;
var defaultLogLevel=setting.log.level;
var defaultLogStream=setting.log.stream;
...
var Log = exports = module.exports = function Log(tag,level,stream){
  //if ('string' == typeof level) level = exports[level.toUpperCase()];
  level = level||defaultLogLevel;
  this.tag=tag||"";
  this.level=exports[level.toUpperCase()];
  this.stream = stream ||defaultLogStream|| process.stdout;
...
Log.prototype = {
  ...
  log: function(levelStr, args) {
    if (exports[levelStr] <= this.level) {
      ...
      this.stream.write(
          '[' + dateFormat(new Date(),defaultDateFormat) + ']'
        + ' ' + levelStr+' '+ this.tag
        + ' - ' + msg
        + '\n'
      );
